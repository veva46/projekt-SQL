...
mezirust AS (
  SELECT 
    srovnani.rok,
    srovnani.kategorie,               --rok 2006 má mezinárust NULL - neexistuje předchozí rok
    srovnani.nazev,                           -- jakostní bílé víno je udáváno až od roku 2015
...
FROM procenta 
  WHERE predchozi_cena IS NOT NULL   
    --AND zmena_ceny = 'zdraženo'          
  ORDER BY rok, kategorie, narust_ceny_procenta ASC;  
...
mezirust AS (
  SELECT 
    ...
    CASE 
   	  WHEN (prumerna_cena - predchozi_cena) < 0 THEN 'zlevněno'
   	  WHEN (prumerna_cena - predchozi_cena) > 0 THEN 'zdraženo' -- oproti předchozímu roku
      WHEN (prumerna_cena - predchozi_cena) = 0 THEN 'stejná cena'        
      ELSE 'nesrovnatelné s předchozím rokem'
    END AS zmena_ceny
   FROM srovnani
