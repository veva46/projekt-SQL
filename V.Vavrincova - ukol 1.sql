...
SELECT       
   ...
   tpr.odvetvi,
   tpr.rok_m,
   ROUND(AVG(tpr.prumer_mzda), 0) AS prumerna_mzda_za_odvetvi,
   LAG(ROUND(AVG(tpr.prumer_mzda), 0)) OVER (      
       PARTITION BY tpr.industry_branch_code ORDER BY tpr.rok_m ) AS predchozi_mzda,
   ROUND(AVG(tpr.prumer_mzda), 0)  
     - LAG(ROUND(AVG(tpr.prumer_mzda), 0)) OVER (
         PARTITION BY tpr.industry_branch_code 
         ORDER BY tpr.rok_m) AS rozdil_mzdy,     --vypočítá rozdíl dvou po sobě jdoucích řádků
   CASE
     WHEN ROUND(AVG(tpr.prumer_mzda), 0) > LAG(ROUND(AVG(tpr.prumer_mzda), 0)) OVER (
            PARTITION BY tpr.industry_branch_code 
            ORDER BY tpr.rok_m 
         ) THEN 'Růst'
     WHEN ROUND(AVG(tpr.prumer_mzda), 0) < LAG(ROUND(AVG(tpr.prumer_mzda), 0)) OVER (
...
FROM trendy_mzda
  ...
  ORDER BY trendy_mzda.trend  DESC, trendy_mzda.rozdil_mzdy DESC, trendy_mzda.rok_m, trendy_mzda.kod_odvetvi;
