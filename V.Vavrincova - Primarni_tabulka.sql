...
WITH 
 ...
 SELECT 
       ...
       EXTRACT(QUARTER FROM cp.date_from::DATE) AS kvartal_p,   -- převede datum do kvartálu v roce
       EXTRACT(YEAR FROM cp.date_from::DATE) AS rok_p,          -- převede datum do roku
 ...
 FROM czechia_price cp
     ...
     GROUP BY cp.category_code, datum_od, datum_do, cpc.name, rok_p, kvartal_p, jednotka
 ),
 mzdy AS (
 SELECT 
...
FROM mzdy
  JOIN potraviny
     ON mzdy.kvartal_m = potraviny.kvartal_p AND mzdy.rok_m = potraviny.rok_p
  ORDER BY mzdy.rok_m, mzdy.kvartal_m, mzdy.odvetvi, potraviny.datum_od, potraviny.potravina;
