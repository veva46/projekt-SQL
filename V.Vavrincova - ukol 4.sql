...
FROM 
  procenta_mzdy 
WHERE 
  predchozi_mzda IS NOT NULL 
  AND zmena_mzdy = 'zvýšena' -- uvažuji jen zvýšení mzdy   
ORDER BY 
  narust_mzdy_procenta DESC;              
...
SELECT 
  ...
  (
    prumer_rustu_cen_potr.prumer_rustu - procenta_mzdy.narust_mzdy_proc
...
FROM 
  procenta_mzdy 
  JOIN prumer_rustu_cen_potr ON procenta_mzdy.rok = prumer_rustu_cen_potr.rok 
WHERE 
  prumer_rustu_cen_potr.predchozi_cena IS NOT NULL --u r. 2006 neexistuje předchozí rok 
  AND procenta_mzdy.predchozi_mzda IS NOT NULL --AND zmena_ceny = 'zdraženo'    -- uvažuji jen zdražení potravin      
  AND (
    prumer_rustu_cen_potr.prumer_rustu - procenta_mzdy.narust_mzdy_proc
  ) > 10 
ORDER BY 
  rozdil_cena_mzda_proc DESC, 
  rok ASC;              
...
mezirust_mzdy AS (
  SELECT 
    ...
    srovnani_mzdy.predchozi_mzda, 
    CASE WHEN (prumer_mzda - predchozi_mzda) < 0 THEN 'snížena' WHEN (prumer_mzda - predchozi_mzda) > 0 THEN 'zvýšena' WHEN (prumer_mzda - predchozi_mzda) = 0 THEN 'stejná' ELSE 'nesrovnatelné s předchozím rokem' END AS zmena_mzdy 
  FROM 
    srovnani_mzdy
), 
procenta_mzdy AS (
  SELECT 
    ...
    ROUND(
      100 *(
        (prumer_mzda - predchozi_mzda)/ predchozi_mzda
      )
    ) AS narust_mzdy_proc 
  FROM 
    mezirust_mzdy
...
FROM 
  procenta_mzdy 
  JOIN prumer_rustu_cen_potr ON procenta_mzdy.rok_m = prumer_rustu_cen_potr.rok_p 
WHERE 
  prumer_rustu_cen_potr.predchozi_cena IS NOT NULL --u r. 2006 neexistuje předchozí rok 
